<?php
/**
 * Description of ddFoursquareVenue
 *
 * @author ner0tic
 */
class ddFoursquareVenue { //extends ddFoursquareBase {
  protected 
            $id             = null,
            $createdAt      = null,
            $name           = null,
  ...
  public static function createFromArray($a = array()) {
      if(sizeof($a)===0)   throw new ddFoursquareException('not an array');
      return new ddFoursquareVenue($a);
  }
...
//    $this->createdAt = time();
    switch(func_num_args()) {
      case 1:
        if(is_array($args)) {
          ...
          $json = json_decode(ddFoursquare::retrieve('venues/'.$args[0]),true);
          
          $this->fromArray($json['response']['venue']);
        }
        break;
      case 2:
        $json = json_decode(ddFoursquareVenue::search('venues/search',array(
            'll'    => is_object($args[1]) ? $args[1]->getCoords() : $args[1]
        )),true);
          $this->fromArray($json['response']['venue']);
        break;
      default:
        throw new ddFoursquareException('invalid parameters supplied');
        break;
    }
  }
  ...
  public static function search($params = array()) {
    return ddFoursquare::retrieve('venues/search', $params);
  }
...
//        $this->$key = $value;
        ...
        if(isset($a['categories']) && (is_array($a['categories'] && sizeof($a['categories'] > 0)))) {
          foreach($a['categories'] as $category)  $this->categories[] = $category; //ddFoursquareCategory::createFromArray($category);
        }
        ...
        if(isset($a['specials']) && (is_array($a['specials'] && sizeof($a['specials'] > 0)))) {
          foreach($a['specials'] as $special)  $this->specials[] = $special; //ddFoursquareSpecial::createFromArray($special);
        }
        ...
        if(isset($a['tips']) && (is_array($a['tips'] && sizeof($a['tips'] > 0)))) {
          foreach($a['tips'] as $tip)  $this->tips[] = $tip; // new ddFoursquareTip($tip);
        }        
        ...
        if(isset($a['specialsNearby']) && (is_array($a['specialsNearby'] && sizeof($a['specialsNearby'] > 0)))) {
          foreach($a['specialsNearby'] as $special)  $this->specialsNearby[] = $special; // new ddFoursquareSpecial($special);
        }              
        
        if(isset($a['photos']) && (is_array($a['photos'] && sizeof($a['photos'] > 0)))) {
          foreach($a['photos'] as $photo)  $this->photos[] = $photo; // new ddFoursquarePhoto($photo);
        }        
